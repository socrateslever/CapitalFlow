...
export const mapLoanFromDB = (l: any, clientsData: any[] = []): Loan => {
  ...
  const installments = rawParcelas.map((p: any) => {
    ...
    return {
      ...
      principalRemaining: p.principal_remaining != null ? asNumber(p.principal_remaining) : undefined,
      interestRemaining: p.interest_remaining != null ? asNumber(p.interest_remaining) : undefined,
      lateFeeAccrued: p.late_fee_accrued != null ? asNumber(p.late_fee_accrued) : undefined,
